...
using Half = WalkerGlobe2.Core.Half;
...
namespace WalkerGlobe2.Renderer
{
    ...
    public static class Device
    {
        static Device()
        {
            // Don't create a window in the static constructor — wait for explicit init
        }
        ...
        /// <summary>
        /// Ensures Device is initialized. Safe to call multiple times.
        /// When called with useCurrentContext=true, uses the already-current GL context
        /// (e.g. from GLWpfControl) instead of creating a temporary GameWindow.
        /// </summary>
        public static void EnsureInitialized(bool useCurrentContext = false)
        {
        ...
        private static void InitFromTempWindow()
        {
            // Use WGL window to avoid GLFW's ~20s startup penalty
            using (GraphicsWindow window = new WglGraphicsWindow(1, 1, "", WindowType.Default))
            {
                QueryGLCapabilities();
                InitLinkUniforms(window.Context.TextureUnits.Count);
                InitDrawUniforms();
                s_textureSamplers = new TextureSamplers();
            }
        }

        private static void InitFromCurrentContext()
        {
            QueryGLCapabilities();

            // TextureUnits count comes from GL query directly
            int numTexUnits;
            GL.GetInteger(GetPName.MaxCombinedTextureImageUnits, out numTexUnits);
            InitLinkUniforms(numTexUnits);
            InitDrawUniforms();
            s_textureSamplers = new TextureSamplers();
        }

        private static void QueryGLCapabilities()
        {
        ...
        private static void InitLinkUniforms(int textureUnitCount)
        {
        ...
        private static void InitDrawUniforms()
        {
            ...
            drawAutomaticUniformFactories.Add(new GlobeModelZToClipCoordinatesUniformFactory());
            ...
            drawAutomaticUniformFactories.Add(new EarthRotationAngleUniformFactory());
            drawAutomaticUniformFactories.Add(new InverseModelMatrixUniformFactory());
        ...
        {
            var window = new WglGraphicsWindow(width, height, title, windowType);
            // After creating a window, its GL context is current — safe to init from it
            EnsureInitialized(useCurrentContext: true);
            return window;
        }

        /// <summary>
        /// Creates a standalone GL context for WPF hosting (no GameWindow).
        /// The GL context must already be current when calling this.
        /// </summary>
        public static Context CreateContext(int width, int height, Action makeCurrent = null)
        {
            EnsureInitialized(useCurrentContext: true);
            return new ContextGL3x(width, height, makeCurrent);
        }
