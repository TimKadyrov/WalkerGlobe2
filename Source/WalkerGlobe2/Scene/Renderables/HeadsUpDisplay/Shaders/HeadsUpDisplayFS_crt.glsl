...
vec4 ToLinear(vec4 c){return vec4(ToLinear1(c.r),ToLinear1(c.g),ToLinear1(c.b),ToLinear1(c.a));}
...
vec4 Fetch(vec2 pos,vec2 off){
  ...
  return ToLinear(texture(og_texture0,pos.xy,-16.0));}
...
vec4 Mask(vec2 pos){
  pos.x+=pos.y*3.0;
  vec4 mask=vec4(maskDark,maskDark,maskDark,maskLight);
...
{
  // Unmodified.
  res=textureSize(og_texture0,0);
  vec4 fragColor=vec4(0,0,0,0);
  vec2 pos=Warp(gl_FragCoord.xy/res.xy+vec2(0.0,0.0));
  fragColor.rgba=Tri(pos)*Mask(gl_FragCoord.xy);
