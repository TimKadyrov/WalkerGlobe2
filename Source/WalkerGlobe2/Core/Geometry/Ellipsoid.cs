...
namespace WalkerGlobe2.Core
{
    public class Ellipsoid
    {
        ...
        public double RadiusAt(double lat)
        {
            var a = _radii.X;
            var b = _radii.Z;
            var d1 = Math.Pow(a * a * Math.Cos(lat), 2.0) + 
                     Math.Pow(b * b * Math.Sin(lat), 2.0);
            var d2 = Math.Pow(a  * Math.Cos(lat), 2.0) +
                     Math.Pow(b  * Math.Sin(lat), 2.0);
